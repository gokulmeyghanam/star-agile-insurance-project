node{
    ...
    stage('git code checkout'){
        try{
            ...
            ${BUILD_URL}''', subject: 'Job ${JOB_NAME} ${BUILD_NUMBER} is failed', to: 'gokulmeyghanam@gmail.com'
        }
    }
    ...
    stage('Containerize the application'){
        ...
        sh "${dockerCMD} build -t gokulmeyghanam/insuraneme:${tagName} ."
    }
    
    stage('Pushing it ot the DockerHub'){
        echo 'Pushing the docker image to DockerHub'
        withCredentials([string(credentialsId: 'dock-password', variable: 'dockerHubPassword')]) {
        sh "${dockerCMD} login -u gokulmeyghanam -p ${dockerHubPassword}"
        sh "${dockerCMD} push gokulmeyghanam/insuraneme:${tagName}"
